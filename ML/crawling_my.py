...
if __name__ == "__main__":
    ...
    news_df = pd.DataFrame({'date':news_date, 'title': news_titles, 'link': final_urls, 'contents': news_contents})
    new_df = news_df.drop_duplicates(keep='first', ignore_index=True)
    print("중복 제거 행 개수: ", len(new_df))
    
    now = datetime.datetime.now()
    news_df.to_csv('{}_{}.csv'.format(search, now.strftime('%Y%m%d+%H%M%S')), encoding='utf-8', index=False)
